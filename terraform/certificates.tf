...
resource "null_resource" "certificates" {
  ...
  provisioner "local-exec" {
    command = "echo '${ data.template_file.certificates.rendered }' > ../ansible/cert/kubernetes-csr.json"
  }

  provisioner "local-exec" {
    command = "cd ../ansible/cert; cfssl gencert -initca ca-csr.json | cfssljson -bare ca; cfssl gencert -ca=ca.pem -ca-key=ca-key.pem -config=ca-config.json -profile=kubernetes kubernetes-csr.json | cfssljson -bare kubernetes"
  }
}
